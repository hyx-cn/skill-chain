---
name: skill-chain
description: Supply chain intelligence for Cursor, Codex, OpenClaw, and OpenCode skills. Use when analyzing the local skill ecosystem, understanding tool and package dependencies, discovering skill categories, mapping relationships between skills, checking security posture, auditing skill health, detecting overlaps, or generating an ecosystem health report. Trigger on "analyze my skills", "skill supply chain", "what tools do my skills use", "skill dependencies", "skill inventory", "ecosystem report", "which skills use X package", "skill categories", "popular skills", "skill security", "skill health", "skill completeness", "overlapping skills".
---

# SkillChain

Supply chain intelligence and ecosystem analysis for Cursor, Codex, OpenClaw, and OpenCode skills. Operates offline-first against locally installed skills; optionally enriches with live clawhub metadata (stars, downloads, moderation verdicts) when network is available.
...
The ingest script checks these locations by default (non-existent paths are skipped):

- **Cursor:** `~/.cursor/skills-cursor`
- **Codex:** `~/.codex/skills`
- **OpenClaw:** `~/.openclaw/skills`, `~/.openclaw/extensions`, `/Applications/OpenClaw.app/Contents/Resources/skills` (macOS)
- **OpenCode:** `~/.opencode/skills`

In addition, `ingest scan` augments the user-provided or default `--dirs` with:

- `<project_root>/skills` — project-local skills folder when this skill lives inside a repository
- `$(npm root -g)/<platform>/skills` — globally installed skills for `openclaw`, `cursor`, `codex`, `opencode` (each appended only if that directory exists)
