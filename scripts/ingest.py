...
# ---------------------------------------------------------------------------
# Default scan roots (multi-platform; non-existent paths are skipped)
# ---------------------------------------------------------------------------
...
def cmd_scan(dirs: list[str]) -> None:
    ...
    # - plus global npm skills dirs for known platforms (when available)
    ...
    # Global npm skills directories: $(npm root -g)/<platform>/skills
    try:
        proc = subprocess.run(
            ["npm", "root", "-g"],
            capture_output=True,
        ...
        if proc.returncode == 0:
            ...
            for platform in ("openclaw", "cursor", "codex", "opencode"):
                global_skills = npm_root / platform / "skills"
